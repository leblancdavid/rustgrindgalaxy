...
- **Procedural animation driver = `PlayerController.Hover.cs`** (partial class), runs in `_Process` and deliberately touches **only** `_visual` `Scale`/`Position`/`Modulate`/`Texture` (plus `_boardVisual.Texture` for the always-on board mist loop) — never `Rotation` — so it won't fight the `_PhysicsProcess` spin/trick code.
