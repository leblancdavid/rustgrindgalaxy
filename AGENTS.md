...
- **Board FX layer = `PlayerController.BoardFx.cs`** (partial): runtime-created `Sprite2D` children of `BoardSprite` — dust (ground-move) and wisps (air) with `ShowBehindParent`, sparks (grind) on top; they inherit the board's flip/tilt/bob/spin, animate only `Texture`/`Position`/`SelfModulate`, and fade via speed-scaled targets. Frames from `res://assets/hoverboards/player/fx/<set>/boardfx_NN.png`; missing set = that FX hidden.
