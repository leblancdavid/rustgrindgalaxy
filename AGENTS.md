...
- Shadows (`Shadow.cs` + `ShadowSilhouette.cs` + `GroundClip.cs` + `ForegroundClip.cs`): every object entity attaches a shadow via `Shadow.Attach(parent)` (index 0 so it draws behind sibling art); effects/projectiles stay shadow-free. Props: `Default` and `Foreground` layers cast (foreground shadow subtrees draw over characters, which matches foreground's role of always occluding them); `Background` dressing never does. Occlusion: foreground-prop visuals register in `ForegroundClip` (lazy world polys, unregistered in `Prop._ExitTree`); a shadow whose caster contact point sits inside one is hidden, so an object drawn behind a foreground prop cannot leak its shadow — the shadow hides and returns exactly with the object. Shadows auto-detect the parent's first textured `Sprite2D`, else first `Polygon2D` body visual (shader-material glows, `Glow`/`Beam`/`Fuse` names are excluded); anything else falls back to the soft ellipse. Silhouettes are baked black, cropped to the tight alpha bbox (sprite art has transparent padding — the flip mirrors about the bottommost *opaque* row so feet stay welded), and vertically flipped; the bake also records where that bottom row sits in the visual's local space, and the shadow anchor starts there when it is *below* the ground hit (props/crates are drawn sunk into the floor for perspective, so their shadows must emerge from the object bottom, not the ground line). Squash and sun shear are runtime-only (`Transform2D` basis, `WorldSun` shear set per level seed). On angled ramps the shadow frame comes from the art's bottom *segment* (`ToGlobal` of the recorded foot corners): anchor = edge midpoint (rotated props have their base displaced sideways from the node center), x-axis = edge direction, y-axis = world-vertical + shear, so the shadow hangs *down the hill* instead of perpendicular to the slope; characters keep the slope-hugging node rotation instead. Animation-frame jitter (per-frame foot-row shift, hover bob, squash pulse): characters anchor their X on the body plus the crop's canvas offset (never the raw art bbox), a foot row only overrides the ground hit when sunk deeper than `FootTrustBand` (player art overlaps the floor ~3.4px max, smallest real prop sink is 5px), and Y + the *world* extents run through a `ContactSmoothing` low-pass that snaps on re-show/teleport — filter the final `texW·sx`/`texH·sy`, not `sx`/`sy`: the tight bbox crop itself changes size between animation frames (walk frames span 24..34px of a 48px canvas), so scale-only filtering leaves the pop unattenuated. Never smooth X — shadows must not lag while running. Ground clips it, it does not gate it: shadows draw wherever the ray hits and are **clipped per-pixel to opaque ground art** by `GroundClip` — it registers the direct `Polygon2D` children of every `LevelTile` (world-space, lazy bake) as ground fill, and each shadow sprite runs an even-odd edge-test shader against per-instance uniforms (canvas_item has no world matrix; Shadow hands it the CPU-composed linear map incl. owner + slope rotation). Catwalk strips keep a partial shadow welded to the strip; nothing hangs over gaps or past platform edges. New tile shapes need no shadow code, but ground art must stay a direct `Polygon2D` of the tile root. Pads/beacons/props are placed flush or sunk into the floor, so a failed or out-of-range direct ray retries from `EmbeddedRetryLift` above the origin and accepts only surfaces within `EmbeddedMaxSunk` (a false accept over a thin platform is still caught by the probe).
- Ramp rail chains (`LevelTile._defaultRailChains` + `RailChainGeometry.cs`): ramp-tile rails are NOT scene data — `TileLevelGenerator.AppendAndPlaceTile` calls `BuildRailChains(entry.Name)` right after assigning `FloorSegments` (before props/supports/palette, which iterate the generated children). Each rail rides one floor segment at a constant PERPENDICULAR clearance (36 default); consecutive rails meet at the miter intersection of the offset lines — `vertex + clearance/cos(Δθ/2)` along the bisector of the segments' up-normals, i.e. `clearance·tan(Δθ/2)` back from the floor vertex. Never go back to a fixed *vertical* offset: perpendicular then collapses as `d·cos θ`, which is exactly why old steep-ramp rails looked glued to the floor. At shoulder corners (slope flattening) the shallow rail miters early and locally dips below nominal clearance over the steep floor (~26 vs 36) — inherent to exact-constant offset, do not "fix" by pinning joints at vertex x. Setting `Width`/`Position`/`Rotation`/snap/speed BEFORE `AddChild` matters (`GrindRail._Ready` sizes visuals/collision from them); chaining uses `SetChainLinks`, not `NextRailPath` (paths resolve in `_Ready` before siblings exist). Full notes + tuning checklist in `docs/TILE_DESIGN.md#rail-chains`.
- Ground-fill materials (`TileTextures.cs` + `ApplyTheme`): the ground shader must NOT rely on baked `Polygon2D.UV` for world-space tiling — writing `poly.UV` on a scene-instantiated polygon (already in the tree, mesh cached) silently has no visible effect (material attached, shader never samples). It works when the polygon is created in code (e.g. the RailShader/BallShader paths). Ground shaders compute the sample position in the vertex stage from `MODEL_MATRIX * VERTEX` (world position, includes the full ancestor chain) so seams stay continuous and no runtime UV bake is needed. **Sampling must wrap the UVs in-shader with `fract(world_uv + vec2(ox, oy))`** — world UVs easily exceed 1.0, and Godot's default texture import is CLAMP, so an unwrapped sample clamps to the art's right/bottom edge texels (which are near-black on the panel textures), silently killing the pattern while raw `t.rgb`/gradient probes still read world coords correctly. `Texture2D.Repeat` does not exist in the C# binding (CS1061), so fract is the fix — never go back to exposing raw UVs. Grayscale textures are normalized ×2 in the shader and blended by a `weight` uniform (0.55) via `COLOR.rgb *= mix(vec3(1.0), clamp(t.rgb * 2.0, 0.0, 1.0), weight)` so a mid-gray (avg ~126) tile never darkens the palette fill to black; branch-free `mix(ta, tb, step(0.5, sel))` picks the a/b texture (ternary works too). Blob-jitter/flip hides the tiling. Per-instance rails keep baked UVs (offset/flip/vertical mapping from C#).
